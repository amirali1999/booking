...
public interface TrainMapper {
    ...
    TrainDTO ObjectToDTO(Train train);
    @Mapping(target = "sourceCity",
            expression = "java(cityRepository.findById(trainDTO.getSourceCityId())" +
                    ".orElseThrow(() -> new BookingException.NotFoundException(\"SourceCityId\")))")
    @Mapping(target = "targetCity",
            expression = "java(cityRepository.findById(trainDTO.getTargetCityId())" +
